...
const TEMPLATE_EMBED = require("./template-embed.js");
const { resolveThemeCss } = require("./theme.js")._internal;
const llm = require("./llm.js");

// Per-build cost ledger. Extracted to lib/cost.js (Sprint 3) — that module owns
// the module-scoped ledger and the OpenAI/Tavily/LLM spend recorders. Same
// singleton semantics (one in-flight generate per instance) and same numbers;
// generate.js just starts the ledger per request and reads its summary.
...
const {
  ...
  classTierKey, classTierSpec, sourceCounts, researchOwner, scoreKnowledgeBase, knowledgeBaseStandard,
...
const {
  ...
  safeHost, KEY_PREFIX, DISCOVERY_RETRY_BACKOFF_MS
...
// The research engine: source-discovery cascade. Extracted to
// lib/core/research-engine.js (Sprint 3, module 5 — step 3/3). Requires util +
// cost + diagnostics + openai; nothing back from generate.js. Imported back into
// this scope so buildSourcePaper (fetchUrlText), the knowledge-core layer, and
// the _internal test export resolve unchanged.
const {
  isPrivateAddress, assertFetchableUrl, fetchUrlText, configuredSearchModels,
  findSourceCandidates, normalizeDiscoveredSources, discoverKnowledgeBaseSources,
  prepareKnowledgeBase, highestMetTier, assessSourceScarcity, buildChangeOrder
} = require("../lib/core/research-engine.js");

// The knowledge-base resolution layer. Extracted to lib/core/knowledge-core.js
// (Sprint 3, module 6). Requires util + openai + research-engine; nothing back
// from generate.js. Imported back so the handler and the _internal test export
// resolve unchanged. resolveKnowledgeBaseLegacy stays in the module (unused here).
const {
  resolveKnowledgeBase, detectAdvancementOpportunity
} = require("../lib/core/knowledge-core.js");

// Source-paper (Student Reader) rendering. Extracted to lib/renderers/source-paper.js
// (Sprint 3, module 7). Requires util + research-engine (fetchUrlText); nothing back
// from generate.js. Imported back into this scope so the buildSourcePaper/sourceText
// call sites (and the paragraphs _internal test export) resolve unchanged. The four
// HTML helpers (sourceLabel/titleCase/sourceQuality/sourceQualityHtml) stay private
// to the module — generate.js never called them directly. sourceQualityFromBody is a
// body-parser, not a renderer helper, so it stays in generate.js.
const {
  buildSourcePaper, sourceText, paragraphs
} = require("../lib/renderers/source-paper.js");
...
function readTemplateFile(name) {
  // Prefer the embedded copy: require() guarantees template-embed.js (and thus
  // every template) is in the serverless bundle, so this never hits ENOENT even
  // when Vercel's file tracer / includeFiles does not ship template/**.
  if (TEMPLATE_EMBED && typeof TEMPLATE_EMBED[name] === "string") {
    return Buffer.from(TEMPLATE_EMBED[name], "base64").toString("utf8");
  }
  // Fallback: read from disk (local dev, or a template not yet embedded).
