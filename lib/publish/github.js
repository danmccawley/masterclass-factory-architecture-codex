...
//
// Dependency injection: publishToGitHub needs slugify() and baseUrl(), which
// remain in generate.js because they are general-purpose helpers used in ~6
// other places there. Requiring generate.js back would be circular, so the
// call site passes them in via a deps object: { slugify, baseUrl }. This keeps
// a single source of truth for those helpers and makes this module's external
...
async function publishToGitHub(req, brief, bundle, deps) {
  const { slugify, baseUrl } = deps;
  const token = String(process.env.GITHUB_TOKEN || "").trim();
